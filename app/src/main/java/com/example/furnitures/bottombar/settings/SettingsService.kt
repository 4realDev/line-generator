package com.example.furnitures.bottombar.settings
...
object SettingsService {
    ...
    fun saveDifficulty(context: Context, difficulty: FurnitureDifficulty) {
        sharedPreferences(context).edit().putString(DIFFICULTY_STORAGE, difficulty.name).apply()
    }

    fun getDifficulty(context: Context): FurnitureDifficulty {
        return FurnitureDifficulty.valueOf(sharedPreferences(context).getString(DIFFICULTY_STORAGE, FurnitureDifficulty.MIDDLE.name) ?: FurnitureDifficulty.MIDDLE.name)
    }
